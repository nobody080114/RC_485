...
* @param:      	void
* @retval:     	void
* @details:    	初始化3个DM4310型号的电机，设置默认参数和控制模式。
*               设置ID、控制模式和命令模式等信息。
************************************************************************
**/
void dm_motor_init(void)
{
	...
	motor[Motor1].mst_id = 0x03;	// 实际没有用上，只做标识作用
	motor[Motor1].tmp.read_flag = 1;
	motor[Motor1].ctrl.mode 	= mit_mode;
	motor[Motor1].ctrl.vel_set 	= 0.0f;		// 设定速度
	motor[Motor1].ctrl.pos_set 	= 0.0f;		// 设定位置
	motor[Motor1].ctrl.tor_set 	= 0.0f;		// 设定前馈力矩
	motor[Motor1].ctrl.cur_set 	= 0.02f;	// 设定电流
	motor[Motor1].ctrl.kp_set 	= 0.0f;		// 设定kp
	motor[Motor1].ctrl.kd_set 	= 0.0f;		// 设定kd
	motor[Motor1].tmp.PMAX		= 25.0f;	// 设定位置最大值
	motor[Motor1].tmp.VMAX		= 30.0f;	// 设定速度最大值
	motor[Motor1].tmp.TMAX		= 10.0f;	// 设定力矩最大值
	
	// 设置Motor2的电机信息
	motor[Motor2].id = 0x01;
	motor[Motor2].mst_id = 0x04;	// 实际没有用上，只做标识作用
	motor[Motor2].tmp.read_flag = 1;
	motor[Motor2].ctrl.mode 	= mit_mode;
	motor[Motor2].ctrl.vel_set 	= 0.0f;		// 设定速度
	motor[Motor2].ctrl.pos_set 	= 0.0f;		// 设定位置
	motor[Motor2].ctrl.tor_set 	= 0.0f;		// 设定前馈力矩
	motor[Motor2].ctrl.cur_set 	= 0.02f;	// 设定电流
	motor[Motor2].ctrl.kp_set 	= 0.0f;		// 设定kp
	motor[Motor2].ctrl.kd_set 	= 0.0f;		// 设定kd
	motor[Motor2].tmp.PMAX		= 25.0f;	// 设定位置最大值
	motor[Motor2].tmp.VMAX		= 30.0f;	// 设定速度最大值
	motor[Motor2].tmp.TMAX		= 10.0f;	// 设定力矩最大值
	
	// 设置Motor3的电机信息
	motor[Motor3].id = 0x02;
	motor[Motor3].mst_id = 0x05;	// 实际没有用上，只做标识作用
	motor[Motor3].tmp.read_flag = 1;
	motor[Motor3].ctrl.mode 	= mit_mode;
	motor[Motor3].ctrl.vel_set 	= 0.0f;		// 设定速度
	motor[Motor3].ctrl.pos_set 	= 0.0f;		// 设定位置
	motor[Motor3].ctrl.tor_set 	= 0.0f;		// 设定前馈力矩
	motor[Motor3].ctrl.cur_set 	= 0.02f;	// 设定电流
	motor[Motor3].ctrl.kp_set 	= 0.0f;		// 设定kp
	motor[Motor3].ctrl.kd_set 	= 0.0f;		// 设定kd
	motor[Motor3].tmp.PMAX		= 25.0f;	// 设定位置最大值
	motor[Motor3].tmp.VMAX		= 5.0f;	// 设定速度最大值
	motor[Motor3].tmp.TMAX		= 10.0f;	// 设定力矩最大值
}
/**
************************************************************************
